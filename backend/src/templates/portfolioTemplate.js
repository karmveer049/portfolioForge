...
function dataJs(p) {
  ...
  return `export const DATA = ${JSON.stringify({
    ...
    academics:   p.academics && typeof p.academics === 'object' ? p.academics : {},
    skillChapters,
    stats,
    projects: toArray(p.projects).map(proj => ({
      title:       proj.title       || '',
      description: proj.description || '',
      stack:       skillsArray(proj.stack),
      liveUrl:     proj.liveUrl     || '',
      githubUrl:   proj.githubUrl   || '',
      screenshot:  proj.screenshot  || '',
      highlight:   proj.highlight   || '',
    })),
    experience: toArray(p.experience).map(exp => ({
      role:        exp.role        || '',
      company:     exp.company     || '',
      ...
      tech:        skillsArray(exp.tech),
    })),
    certifications: toArray(p.certifications).map(c => ({
      title: c.title || '',
      org:   c.org   || '',
      url:   c.url   || '',
    })),
    achievements: toArray(p.achievements).map(a => ({
      title: a.title || '',
      desc:  a.desc  || '',
    })),
  }, null, 2)}\n`
